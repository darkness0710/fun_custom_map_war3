...
local function selfTest()
  ...
  API.after(CFG.SYNC_TEST_WAIT, function()
    ...
    API.trace("sync: tu kiem [" .. S.sync.mode .. "] ve=" ..
      (#ve > 0 and table.concat(ve, ",") or "khong ai") ..
      " mat=" .. (#mat > 0 and table.concat(mat, ",") or "khong ai"))
...
local function statusLines()
  ...
  local ve = {}
  for i = 1, #S.pids do
    if S.sync.pingSeen[S.pids[i]] then ve[#ve + 1] = S.pids[i] end
  end
  out[#out + 1] = "Ping ve tu player: " ..
    (#ve > 0 and table.concat(ve, ", ") or CFG.C_RED .. "khong ai" .. CFG.C_END)
  return out
end
