...
CFG_LUA = os.path.join(ROOT, "src", "1_nen", "1_config.lua")
LANG_LUA = os.path.join(ROOT, "src", "1_nen", "6_lang.lua")
...
class Curve(object):
    ...
    def heSo(self, sk, lv): return sk.get("heSo", 0.0) * self.dmg ** (lv - 1)
    def pct(self, sk, lv):  return sk.get("pct", 0.0) * self.passive ** (lv - 1)
    def cdAt(self, sk, lv): return sk.get("cd", 0.0) * self.cd ** (lv - 1)
    def giapAt(self, sk, lv): return sk.get("giap", 0.0) * self.passive ** (lv - 1)
...
def ten_of(sk, lang):
    return (sk.get("en") or sk.get("ten")) if lang == "en" else \
           (sk.get("ten") or sk.get("en"))
...
def tooltip(sk, lv, cur, lang, T):
    ...
    dong = []
    mota = mota_of(sk, lang) or ""
    if "%s" in mota:
        mota = mota.replace("%s", cur.fmt(sk, lv))
    mota = mota.replace("%%", "%")
    dong.append(mota)
    dong.append("")

    bac = ("Level %d/%d" if lang == "en" else "Bac %d/%d") % (lv, cur.maxlv)
    dong.append(bac)
...
def wts_write(path, raw, new):
    ...
    shutil.copyfile(path, os.path.join(BAK_DIR, "war3map.wts.goc"))
...
def build_plan(lang):
    ...
    chu = [s for s in sks if s.get("loai") == "chudong"]
    bi  = [s for s in sks if s.get("loai") != "chudong"]
    if len(chu) > len(O_CHUDONG) or len(bi) > len(O_BIDONG):
        die("Hart co %d chu dong / %d bi dong -- command card chi co %d/%d o"
            % (len(chu), len(bi), len(O_CHUDONG), len(O_BIDONG)))

    o = {}
    for i, s in enumerate(chu):
        o[s["id"]] = O_CHUDONG[i]
...
def cmd_show(lang):
    ...
    for it in plan:
        print("  %s  %-14s  o (%d,%d)" % (it["id"], it["ten"], it["o"][0], it["o"][1]))
    print("\n--- tooltip mau: %s bac 1 va bac %d ---" % (plan[0]["ten"], cur.maxlv))
...
def cmd_gen(map_dir, lang):
    ...
    print("[ok] xong. Ban cu o build/war3map.w3a.goc va build/war3map.wts.goc")
